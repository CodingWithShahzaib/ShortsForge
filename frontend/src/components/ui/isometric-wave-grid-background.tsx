...
interface IsoLevelWarpProps extends React.HTMLAttributes<HTMLDivElement> {
  /**
   * Primary line color (RGB values as string, e.g. "14, 165, 233").
   * Default: cyan/sky-500
   */
  color?: string;
  /**
...
const IsoLevelWarp = ({
  ...
  return (
    <div
      ref={containerRef}
      className={cn("absolute inset-0 z-0 overflow-hidden bg-white dark:bg-black", className)}
      {...props}
    >
      <canvas ref={canvasRef} className="block w-full h-full" />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,#ffffff_100%)] opacity-20 dark:bg-[radial-gradient(circle_at_center,transparent_0%,#000_100%)] dark:opacity-60 pointer-events-none" />
    </div>
  );
};
